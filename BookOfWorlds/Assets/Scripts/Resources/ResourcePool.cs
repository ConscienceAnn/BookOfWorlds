...
public class ResourcePool : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        Debug.Log($"[ResourcePool] Awake() for {gameObject.name}, resource: {resourceData.resourceName}, ÔÂÙ‡·Ó‚: {prefabs.Length}");
    ...
    private GameObject CreateNewObject()
    {
        ...
        {
            vs.ForceRefresh();
            Debug.Log($"[ResourcePool] VisualState.ForceRefresh() ‚˚Á‚‡Ì ‰Îˇ {obj.name}");
        }
        else
        {
        ...
        if (source != null)
        {
            ...
            source.SetColored();
            source.Show();
            Debug.Log($"[ResourcePool] ResourceSource Ì‡ÒÚÓÂÌ ‰Îˇ {obj.name}");
        }
    ...
    public GameObject Get(Vector3 position, Quaternion rotation)
    {
        ...
        if (obj != null)
        {
            ...
            {
                source.Show();
                Debug.Log($"[ResourcePool] ResourceSource.Show() ‚˚Á‚‡Ì ‰Îˇ {obj.name}");
            }
            else
            {
